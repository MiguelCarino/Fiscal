...
const I18N = {
    ...
    es: {
        ...
        'No backup has been taken from this device yet.': 'Todavía no se ha hecho ninguna copia de seguridad desde este dispositivo.',
        'Check a number': 'Comprobar un número',
        'Paste a number, or one per line. Nothing here is saved to this device.': 'Pega un número, o uno por línea. Nada de esto se guarda en este dispositivo.',
        'Check': 'Comprobar',
        'No jurisdiction this app knows issues a number in that shape.': 'Ninguna jurisdicción que esta aplicación conoce emite un número con esa forma.',
        'The last backup was {date} and this device has changed since.': 'La última copia de seguridad fue el {date} y este dispositivo ha cambiado desde entonces.',
        'This browser has not promised to keep this data, so it can be cleared to make room.': 'Este navegador no ha prometido conservar estos datos, así que puede borrarlos para hacer espacio.',
    ...
    'pt-BR': {
        ...
        'No backup has been taken from this device yet.': 'Ainda não foi feita nenhuma cópia de segurança a partir deste aparelho.',
        'Check a number': 'Verificar um número',
        'Paste a number, or one per line. Nothing here is saved to this device.': 'Cole um número, ou um por linha. Nada disto é guardado neste aparelho.',
        'Check': 'Verificar',
        'No jurisdiction this app knows issues a number in that shape.': 'Nenhuma jurisdição que esta aplicação conhece emite um número com esse formato.',
        'The last backup was {date} and this device has changed since.': 'A última cópia de segurança foi em {date} e este aparelho mudou desde então.',
        'This browser has not promised to keep this data, so it can be cleared to make room.': 'Este navegador não prometeu guardar estes dados, portanto pode apagá-los para liberar espaço.',
    ...
    ja: {
        ...
        'No backup has been taken from this device yet.': 'この端末からはまだバックアップが作成されていません。',
        'Check a number': '番号を確認',
        'Paste a number, or one per line. Nothing here is saved to this device.': '番号を貼り付けてください。1行に1件でも構いません。ここに入力したものはこの端末に保存されません。',
        'Check': '確認',
        'No jurisdiction this app knows issues a number in that shape.': 'この形式の番号を発行している法域は、このアプリの中にはありません。',
        'The last backup was {date} and this device has changed since.': '最後のバックアップは{date}で、それ以降この端末の内容は変わっています。',
        'This browser has not promised to keep this data, so it can be cleared to make room.': 'このブラウザはこのデータを保持すると約束していないため、空き容量のために消される可能性があります。',
    ...
    ru: {
        ...
        'No backup has been taken from this device yet.': 'С этого устройства ещё ни разу не делали резервную копию.',
        'Check a number': 'Проверить номер',
        'Paste a number, or one per line. Nothing here is saved to this device.': 'Вставьте номер или по одному в строке. Ничего отсюда не сохраняется на это устройство.',
        'Check': 'Проверить',
        'No jurisdiction this app knows issues a number in that shape.': 'Ни одна известная приложению юрисдикция не выдаёт номер такой формы.',
        'The last backup was {date} and this device has changed since.': 'Последняя резервная копия — {date}, и с тех пор это устройство изменилось.',
        'This browser has not promised to keep this data, so it can be cleared to make room.': 'Этот браузер не обещал хранить эти данные, поэтому может удалить их, чтобы освободить место.',
